...
<div id='info' align="center" style='margin-top:10%;position:absolute;z-index:1;background:#EEFFEE;box-shadow:0px 0px 5px black;min-width:100%;'></div>
	...
	<?php
				...
				if(isset($_GET['view']))           //If something is exists, then it is set and we will do something 
				{
					$view = $_GET['view'];
					...
					if($view!="")
					{
						...
						else
						{
							...
							$endpos = strpos($bigData, "\n", $startpos); // strpos may return either FALSE or 0
							$resultStr = substr($bigData, $startpos, $endpos - $startpos); //The substr function returns a part of a string
							
							$row = explode("***", $resultStr);   //The explode function in PHP is used to split a string into multiple strings based on the specified delimiter
							...
							$city = trim($row[12]);      //The trim function removes whitespace and other predefined characters from both sides of a string
							...
							elseif($namelength > 20 and $namelength<24)
							{
									$width = '160px';
							}
							else
